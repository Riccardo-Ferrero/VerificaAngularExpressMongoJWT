...
const C_STUDENTS = "libri"; // collection degli studenti
...
app.post("/api/login", (req, res) => {
    ...
    tokenAdministration.createToken(req.body);
...
// ----------------------------------------------------------
app.get("/api/libri", checkToken, (req, res) => {
...
// ----------------------------------------------------------
app.post("/api/libri/cercaPerGenere", checkToken, (req, res) => {
    mongoFunctions.find(DB,C_STUDENTS,req.body,(err,data)=>{
        if(err.codeErr == -1)
            res.send({data:data, newToken:req.newToken});
        else
...
// ----------------------------------------------------------
app.post("/api/libri/inserisci", checkToken, (req, res) => {
    mongoFunctions.insert(DB,C_STUDENTS,req.body,(err,data)=>{
        if(err.codeErr == -1)
            res.send({msg:"Libro inserito", id:data.insertedId, newToken:req.newToken});
        else
            sendError(res,err.codeErr,err.message);
    });
});
